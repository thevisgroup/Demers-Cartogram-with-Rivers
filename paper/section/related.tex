...
\citea{nusrat2016State} define and summarize three major accuracy dimensions for cartograms: statistical, geographical, and topological. Each cartogram design may make various types of accuracy trade-offs between dimensions. We provide a comparison of these trade-offs as introduced by \citea{nusrat2016State} in Table \ref{table:accuracy}. In addition, we include the Demers as it is the focus of our work.
...
\begin{table*}[!tb]
	...
	\resizebox{\textwidth}{!}{
		\begin{tabulary}{\textwidth}{|*{4}{l|}r|c|}
			...
			\hline
			\citea{nickel2022Multicriteriaa} & \citetitlea{nickel2022Multicriteriaa} & Contiguous, Demers & US, Netherlands, World & 49 - 342 & \citeyear{nickel2022Multicriteriaa} \\
			\hline
...
\thumbnail{dorling}{
	...
	In a Demers cartogram, a variant of Dorling, squares are used instead to capture a certain level of topology, as described by \citea{cano2015Mosaic} in their related work section.
...
\bobgraph{Peer-reviewed Applications: }\citea{warf2008Geography} use a Dorling cartogram to represent religious diversity in the US. \citea{sun2010Effectiveness} depict 1996 US election data and 2005 Chinese population data using Dorling, Mosaic, and contiguous cartograms. \citea{gao2020Visualising} present a Dorling cartogram to illustrate COVID-19 infections in China. \citea{tong2018Cartograms} use a Demers cartogram to visualize health-related data by Clinical Commissioning Group (CCG) regions in England, the work introduces a novel technique to remove the overlap of squares based on topological features, aiming to improve both geographical and topological accuracy. \citea{nusrat2020Recognition} investigate the memorability of contiguous and Dorling cartograms by using multiple datasets including demographics, agriculture, and retail data in the US. See \Cref{table:region vs node} for a list of literature that adopts cartograms for visualization with corresponding geographical regions and node counts.

Our work extends the algorithm described by \citea{tong2018Cartograms} which incorporates a static topological feature into Demers cartograms. Our work enhances that of \citeauthor{tong2018Cartograms} in multiple ways. First we introduce multiple features (rivers) into the layout, as opposed to a single river. Second, we make topological features dynamic and further improve the legibility and geographical accuracy. By the term \textit{dynamic}, we mean that the position of the rivers is updated as part of the layout algorithm. In previous work the river is static and merely serves as a boundary. Third, we improve the algorithm for resolving stalemates. Finally, the way we evaluate the cartograms is also different. \citeauthor{tong2018Cartograms} count river crossings to evaluate error (a statistical metric). Here our focus is on readability, thus we include a user study.
...
One of the disadvantages of Dorling and Demers cartograms is legibility. The layout algorithms may displace regions far from their original position and make the maps more difficult to interpret. \citea{nickel2022Multicriteria} present a method to compute stable Demers cartograms with multiple constraints to maintain adjacencies with no overlapping nodes.

In this paper, we introduce a new type of topological feature, a river, as a constraint to compute the final layout, aiming to improve the interpretation, readability, and accuracy of this class of cartograms.