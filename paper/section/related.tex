...
\new{While we focus on rectangular and Dorling cartograms in this paper, we provide brief definitions of contiguous and non-contiguous cartograms for interested readers: Contiguous cartograms are topology-preserving that maintain connectivity with their adjacent areas but have distortion in shape. Non-contiguous cartograms sacrifice the topological connectivity to enable growing or shrinking in size to maintain their original shape \cite{dent2009Cartography}.}

\citea{nusrat2016State} defines summarize three major accuracy dimensions for cartograms: statistical, geographical, and topological. Each cartogram design may make accuracy trade-offs between dimensions, we provide an overview of these cartograms in Table \ref{table:accuracy}.
...
Compared to previous work on Demers cartograms, we introduce dynamic topological feature, a river, aiming to improve the interpretation, readability, and topological accuracy.