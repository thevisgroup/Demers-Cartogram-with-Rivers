...
\newcommand{\thumbnail}[2]{
    \begin{samepage}
      ...
      \begin{wrapfigure}{r}{0.4\columnwidth}
        \vspace{-15pt}
        \begin{center}
          \includegraphics[width=0.38\columnwidth]{"#1".png}
        \end{center}
      \end{wrapfigure}
      #2
...
\author{Qiru Wang\affilnum{1}, Kai Xu\affilnum{1} and Robert S. Laramee\affilnum{1}}

\affiliation{\affilnum{1}University of Nottingham, UK}

\email{qiru.wang1@nottingham.ac.uk}
...
\keywords{Cartogram, Demers Cartogram, EHR Visualization, Visual Analytics}
...
One of the drawbacks of Demers cartograms is that they may become more difficult to read when a region becomes displaced from its geospatial origin through the node layout process.
...
Standard Demers cartograms are composed of square nodes that represent geographic enumeration units.
...
\bobgraph{Peer-reviewed Applications: } There has been a substantial amount of research done in this area, here we review some of the important work that has inspired our work. \citet{warf2008Geography} use a Dorling cartogram to represent religious diversity in the US.
...
To illustrate this more clearly, we refer to the video demonstration published by \citet{tong2018Cartograms}. Because the behavior of the layout algorithm is dynamic, a video is more appropriate to convey the motivation of dynamic features of interest.
We can observe the cartographic layout algorithm at 1:30 and 2:40 of the video by \citet{tong2018Cartograms}.
During the update process, we can clearly observe nodes cross the River Thames as the size expands.
As the size of all the nodes expands, some nodes are pushed south to make more use of screen space.
If we want to make the most efficient use of screen space, we need to translate the River Thames further south during the layout process to prevent nodes from crossing it.
This is one of the main motivations for introducing dynamic rivers. 
By introducing river translations, our implementation prevents nodes from crossing rivers, thereby improving screen space efficiency and cartogram legibility.
For comparison, see our video demonstration (from 0:28 to 0:32 and from 1:49 to 1:56)\footnotemark{}.

\footnotetext{Demonstration of our layout algorithm's implementation at \url{https://youtu.be/PRNEF3J1hl0}.}
...
The Washington Post uses cartograms to visualize the US overseas economic assistance, in arms sales (Mosaic) \cite{bearak2016Everything}, the 2016 US Election (contiguous) \cite{gamio2016Election}, and the Brexit Referendum (Mosaic) \cite{taylor2016What}.
...
\bobgraph{River Shapefiles:} We used OpenStreetMap \cite{openstreetmapRelation} as our data source to obtain shapefiles for the River Thames, the Trent River, and the Great Ouse River in England.
...
See \Cref{dataset} for a detailed description of the datasets used.
...

{
    \begin{figure*}[tb!]
        ...
        \caption{An overview of our hybrid layout algorithm incorporating rivers.
            ...
            For illustration purposes, we show the rendered views alongside the logical views representing the actual computation, and use the same size for all squares for clarity.}
        \label{fig:flowchart}
    \end{figure*}
}
...
\bobgraph{Initialization with Rivers:} We first load and (optionally) render the CCG geospatial boundaries.
...
\begin{figure}[tb!]
    ...
    See \Cref{subsec:{Process Stalemates}}, \Cref{fig:stalemate}, and \Cref{fig:corridor} for more detail. Refer to our video demonstration (from 1:27 to 1:46)\footnotemark{} for details.}
    \label{fig:flowchart-stalemate}
\end{figure}
}
\footnotetext{Demonstration for Demers Cartogram with Rivers at \url{https://www.youtube.com/watch?v=DgCwCkyfGKk}.}
...
A longer corridor length allows more nodes to translate during a stalemate.
...
Similarly, increasing the resolution of rivers, at the expense of the speed of node-river intersection test, may produce a layout with higher legibility.

    {
        \begin{figure*}[tb!]
            ...
            User options are provided to adjust the terminating conditions (size and error), color mapping, and visibility of nodes and rivers.
...

    {
        \begin{figure}[tb!]
            ...
            \caption{A sample location task for participants.
                ...
                Both images show the three longest rivers in England, with the size and color of nodes representing the prevalence of the selected disease.
...
Instructional videos are available at \url{https://tinyurl.com/demerscartogram}.

\textbf{P2:} The participants are given three practice tasks to familiarize themselves with the user study design.
A sample task is shown in \Cref{fig:task}, and accessible at \url{https://ghr.wangqiru.com/#/P1} and included in the online materials\footnotemark{}.
The practice tasks are identical to the actual tasks, accompanied by instructions provided before the actual tasks commence.
...
\footnotetext{The materials for P1-P4 are available on the Open Science Foundation website at \url{https://osf.io/q39w7} and \url{https://github.com/thevisgroup/Demers-Cartogram-with-Rivers}.}
...
However, the average time to locate a given CCG is reduced for 3 out of the 4 CCGs in the study.
...
We would like to explore the case of river-river intersections (or confluence) and test out more geographies such as the U.S.
...
% \begin{noindent}
    ...
    \begin{algorithm}[H]
        \caption{Procedure to derive a point based on an edge and a distance.}\label{alg:derive corridor point}
        \textbf{Input:} \\
...
% \begin{noindent}
    ...
    \begin{algorithm}[H]
        \caption{Procedure to derive a corridor to resolve stalemates.
