...
\begin{abstract}
    ...
    \new{As a variant of} the Dorling cartogram, the Demers cartogram utilizes squares instead of circles to represent regions.
...
\new{In other words, Demers cartograms are useful when a data dimension is not describing the geography of the region it represents but is tied to something else, for example the health of its population.}
...
\new{See \citea{nickel2022Multicriteria} for a more complete description of the advantages that Demers cartograms offer.}
...
Standard Demers cartograms are composed of square nodes that \new{representing geographic enumeration units}.
As such, this can reduce their legibility.
We implement a new hybrid cartographic layout algorithm that combines rivers with the placement of the nodes \new{representing geographic enumeration units, in this case, a Clinical Commissioning Group (CCG)}.
...

    {
        ...
        \begin{table*}[!tb]
            ...
            \caption{Related work with non-contiguous cartogram-based visualizations.
                ...
                \textbf{Number of nodes} is the number of nodes \new{(representing geographic enumeration units)} depicted in the cartogram.
            }
...
\bobgraph{Peer-reviewed Applications: } \new{There has been a substantial amount of research done in this area, here we review some of the important work that has inspired our work}.\citet{warf2008Geography} use a Dorling cartogram to represent religious diversity in the US.
...
\bobgraph{Cartograms in Media: }Cartograms \new{are an engaging visual representation and therefore they are a} popular choice of representation in covering various topics by the media.
...
One iteration of the layout ends when 1) no node overlap is present; \new{AND} 2) no nodes cross a river.
...
\new{For a detailed derivation of the formula, refer to the work by \citet{alam2015Quantitative}.}
...
We conduct a user study to evaluate the effectiveness of our approach.
...
Given a standard choropleth on the left side of the screen and the \new{corresponding} cartogram on the right side, we ask user study participants to locate the given region in the choropleth (on the left) in the cartogram (on the right).
See \Cref{fig:task}.
...
We set this parameter to render rivers or not.
...
\bobgraph{Control Variables: }\textit{Choice of color map:} We use D3's built-in interpolateRdYlGn color map, a diverging color scheme of red, yellow, and green, to depict the data in our cartograms.
...
The color of both visual designs is mapped to the dataset.
...
\new{Due to pandemic restrictions, we designed the user study to be carried out online.
Participants are expected to use their personal hardware for the tasks.
However, devices with small screens are not recommended because of the large size of the cartograms that are rendered.}
The user study includes four parts:
...
\textbf{P3:} The participants are asked to complete 16 location tasks that involve 4 CCGs.
...
\begin{figure}[b!]
    ...
    \caption{In the bar chart, the y-axis shows the average accuracy rate of locating CCGs with and without rivers for user study participants.
        The x-axis shows the target CCGs.}
    \label{fig:task-acc}
\end{figure}
}
...
Currently, the runtime of our layout algorithm is approximately 30 milliseconds for each iteration. \new{ When the quantity of nodes and features increases, generating the optimal layout demands several hundred iterations.}
...
We would like to explore the case of river-river intersections \new{(or confluence)} and testing out more geographies such as the U.S.
...
\noindent\textbf{Improved User Study:} \new{Due to pandemic restrictions, the user study did not take place in a controlled environment.
In the future, we aim to execute a more structured study, ensuring a controlled environment and standardized hardware for participants.
}
