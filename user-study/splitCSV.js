...
// with river, disallow crossing
const group1 = {
	name: "group1",
	tasks: taskList.filter((task) => task.withRiver && !task.allowCrossing).map((task) => task.id),
	accuracy: [],
	response: [],
};

// with river, allow crossing
const group2 = {
	name: "group2",
	tasks: taskList.filter((task) => task.withRiver && task.allowCrossing).map((task) => task.id),
	accuracy: [],
...
const groupAll = {
	name: "groupAll",
	tasks: taskList.map((task) => task.id),
	accuracy: [],
	response: [],
};
...
let rowCount = 0;
function readCSV() {
	fs.createReadStream("/Users/henry/Git/PhD/GHRVis/user-study/result.csv")
		.pipe(parse({ delimiter: ",", from_line: 1 }))
		.on("data", (row) => {
			for (let i = 0; i < taskList.length; i++) {
				const time = Number.parseInt(row[i * 2 + 1]) / 1000;
		...
		.on("end", () => {
			...
			let w = result
				.filter((row) => !row.allowCrossing && row.withRiver)
				.map((row) => row.rt);
			let wo = result
				.filter((row) => !row.allowCrossing && !row.withRiver)
				.map((row) => row.rt);
				...
				if (group.name != "groupAll") {
					...
					for (let i = 0; i < group.tasks.length / 2; i++) {
						if (group.name == "group2" || group.name == "group3") {
							...
							group.accuracy[i][1] =
								group.accuracy[i + group.tasks.length / 2][1];
							group.response[i][1] =
								group.response[i + group.tasks.length / 2][1];
						} else {
							group.accuracy[i][2] =
								group.accuracy[i + group.tasks.length / 2][1];
							group.response[i][2] =
						...
						group.accuracy[i][0] = taskList.find(
							(task) => task.id === group.accuracy[i][0],
						).ccg;
						group.response[i][0] = taskList.find(
							(task) => task.id === group.response[i][0],
						).ccg;
...
function writeCSV(name, columns, data) {
	const writableStream = fs.createWriteStream(`/Users/henry/Git/PhD/GHRVis/user-study/${name}.csv`);
